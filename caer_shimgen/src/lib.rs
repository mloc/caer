...
impl<'ir, W: Write> Generator<W> {
    ...
    pub fn generate(&mut self, ir: &Env) -> Result<()> {
        self.write("// AUTOGENERATED SHIM CODE\n")?;
        for dty in ir.type_tree.types.iter() {
    ...
    fn write_type(&mut self, ir: &Env, dty: &DType) -> Result<()> {
        self.write_path(dty)?;
        self.write("{ \\\n")?;
    ...
    fn write_path(&mut self, dty: &DType) -> Result<()> {
    ...
    fn write_proc(&mut self, ir: &Env, proc: &ProcInfo) -> Result<()> {
